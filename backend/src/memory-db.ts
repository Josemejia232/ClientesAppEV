...
function datetime(now: string, modifier?: string): string {
  const d = new Date();
...
function dateFn(now: string): string {
  const d = new Date();
...
function evalExpr(expr: string, row: any, params: any[]): any {
  expr = expr.trim();
  if (expr === '1') return 1;
  if (expr === '1=1') return true;
  if (expr.startsWith("'") && expr.endsWith("'")) return expr.slice(1, -1);
  if (expr === '?') return params.shift();
  if (expr.toUpperCase() === 'TRUE') return true;
  if (expr.toUpperCase() === 'FALSE') return false;
  if (!isNaN(Number(expr))) return Number(expr);
  if (expr.includes('datetime(') || expr.includes("'now'")) return datetime('now', 'localtime');
  if (expr.startsWith("date('now')")) return dateFn('now');
  if (expr.includes('julianday(')) {
    const match = expr.match(/julianday\(['"]?(.+?)['"]?\)/);
    if (match) return julianday(evalStr(match[1], row));
    return 0;
  }
...
function evalStr(str: string, row: any): string {
  str = str.trim();
...
function parseWhere(sql: string): { condition: string; params: string[] } {
  const whereMatch = sql.match(/WHERE\s+(.*?)(?:ORDER BY|GROUP BY|LIMIT|$)/i);
  if (!whereMatch) return { condition: '1=1', params: [] };
  return { condition: whereMatch[1].trim(), params: [] };
}

function evaluateCondition(condition: string, row: any, params: any[]): boolean {
  ...
  return orParts.some(orPart => {
    ...
    return andParts.every(part => {
      part = part.trim();
      if (part === '1=1' || part === '1') return true;
      ...
      if (part.includes('=')) {
        ...
        const rv = row[col];
        if (val === null || val === undefined) return rv === null || rv === undefined || rv === '';
...
function parseColumns(sql: string): string[] {
  ...
  cols.split(',').forEach(c => {
    ...
    if (asMatch) aliases.push(asMatch[2] || asMatch[1]);
  });
  return aliases;
}

function parseFrom(sql: string): { table: string; join?: { type: string; table: string; on: string } } {
  const fromMatch = sql.match(/FROM\s+(\w+)(?:\s+(\w+))?(?:\s+(LEFT\s+)?JOIN\s+(\w+)(?:\s+(?!ON\b)(\w+))?\s+ON\s+(.+?))?(?:\s+WHERE|\s+ORDER|\s+GROUP|\s+LIMIT|$)/i);
  if (!fromMatch) return { table: '' };
...
function parseOrderBy(sql: string): { field: string; dir: string }[] {
...
function toSqlDate(): string {
  return new Date().toISOString().replace('T', ' ').replace(/\.\d+Z$/, '');
}

function toSqlDateOnly(): string {
    ...
    get: (...allParams: any[]): any => {
      ...
      if (op === 'SELECT') {
        ...
        if (isCount) {
          ...
          return { [aliasMatch?.[1] || 'count']: filtered.length } as any;
        }
        if (join) {
      ...
      if (op === 'INSERT') {
        ...
        if (tableName && colsMatch && valsMatch) {
          ...
          const vals = valsMatch[1].split(',').map(v => {
            const vt = v.trim();
            if (vt === '?') return params.shift();
            if (vt.startsWith("'") && vt.endsWith("'")) return vt.slice(1, -1);
            if (vt.startsWith('"') && vt.endsWith('"')) return vt.slice(1, -1);
            if (vt.match(/datetime/i)) return toSqlDate();
            if (vt.match(/date\(/i)) return toSqlDateOnly();
            if (vt === 'NULL' || vt === 'null') return null;
            return isNaN(Number(vt)) ? vt : Number(vt);
          });
          const row: any = {};
          cols.forEach((c, i) => { row[c] = vals[i]; });
          row.id = nextId[tableName as keyof typeof nextId]++;
        ...
        if (tableName && setMatch) {
          ...
          filtered.forEach(r => {
            sets.forEach(st => {
              ...
              if (vt === '?') r[col] = params.shift();
              else if (/datetime/i.test(vt)) r[col] = toSqlDate();
              else if (/date\(/i.test(vt)) r[col] = toSqlDateOnly();
              else if (vt.startsWith("'") && vt.endsWith("'")) r[col] = vt.slice(1, -1);
              else { const n = Number(vt); r[col] = isNaN(n) ? vt : n; }
            });
          });
    ...
    all: (...allParams: any[]): any[] => {
      ...
      if (op === 'SELECT') {
        ...
        if (groupMatch) {
          ...
          const groups: Record<string, { count: number; item: any }> = {};
          filtered.forEach(r => {
            const key = String(r[groupCol] ?? '');
            if (!groups[key]) groups[key] = { count: 0, item: r };
            groups[key].count++;
          });
        ...
        orderBys.forEach(ob => {
          ...
          filtered.sort((a, b) => {
            const av = a[field] ?? '';
            const bv = b[field] ?? '';
            if (typeof av === 'number' && typeof bv === 'number') return ob.dir === 'ASC' ? av - bv : bv - av;
  ...
  const exec = (_sql: string): any[] => {
    ...
    if (s.startsWith('CREATE TABLE')) {
      ...
      if (tableMatch) {
        ...
        if (tableName === 'clients') nextId.clients = 1;
        if (tableName === 'notas') nextId.notas = 1;
        if (tableName === 'programaciones') nextId.programaciones = 1;
      }
      return [];
    }
    if (s.startsWith('CREATE INDEX')) return [];
      ...
      if (tableName && setMatch) {
        ...
        filtered.forEach(r => {
          sets.forEach(st => {
            const [col, valExpr] = st.split('=').map(x => x.trim());
            const vt = valExpr.trim();
            if (vt.match(/datetime/i)) r[col] = toSqlDate();
            else if (vt.match(/date\(/i)) r[col] = toSqlDateOnly();
          });
        });
      }
      return [];
